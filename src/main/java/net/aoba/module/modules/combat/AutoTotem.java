...
public class AutoTotem extends Module implements ReceivePacketListener, TickListener {
    ...
    // Đã xóa .min(1).max(9) để tránh lỗi compile
    private final IntegerSetting totemSlot = IntegerSetting.builder().id("totem_slot").displayName("Totem Slot (1-9)").defaultValue(1).build();
    ...
    // Đã xóa .min(1).max(9)
    private final IntegerSetting crystalSlot = IntegerSetting.builder().id("cry_slot").displayName("Crystal Slot (1-9)").defaultValue(3).build();
    private final IntegerSetting obsidianSlot = IntegerSetting.builder().id("obi_slot").displayName("Obsidian Slot (1-9)").defaultValue(2).build();
    ...
    // Đã xóa .min(1).max(9)
    private final IntegerSetting anchorSlot = IntegerSetting.builder().id("anc_slot").displayName("Anchor Slot (1-9)").defaultValue(8).build();
    private final IntegerSetting glowstoneSlot = IntegerSetting.builder().id("glo_slot").displayName("Glowstone Slot (1-9)").defaultValue(9).build();
    ...
    // Đã xóa .min(1).max(9)
    private final IntegerSetting pearlSlot = IntegerSetting.builder().id("pearl_slot").displayName("Pearl Slot (1-9)").defaultValue(4).build();
    ...
    private void handleTotemRefill() {
        ...
        int totemIdx = getSlotIndex(totemSlot);

        switch (currentTotemStep) {
            case SELECT_SLOT:
                if (now - lastTotemTime >= 50) {
                    setHotbarSlot(totemIdx);
            ...
            case OPEN_INV:
                ...
                if (now - lastTotemTime >= delay) {
                    currentTotemStep = TotemStep.FIND_SWAP;
                    lastTotemTime = now;
                }
                break;
            ...
            case FIND_SWAP:
                if (mc.currentScreen instanceof InventoryScreen) {
                    if (now - lastTotemTime >= delay) {
                        ...
                        if (targetSwapSlot != -1) {
                            aimSlot(targetSwapSlot);
                            currentTotemStep = TotemStep.DO_SWAP;
                        } else {
                            resetAll();
                        }
                        lastTotemTime = now;
            ...
            case DO_SWAP:
                if (mc.currentScreen instanceof InventoryScreen) {
                    if (now - lastTotemTime >= delay) {
                        ...
                        currentTotemStep = TotemStep.FIND_REFILL;
                        lastTotemTime = now;
                    }
                }
                break;
            ...
            case FIND_REFILL:
                if (mc.currentScreen instanceof InventoryScreen) {
                    if (now - lastTotemTime >= delay) {
                        if (isSlotTotem(36)) {
                            // Hotbar slot đã có totem
                            if (autoEsc.getValue()) {
                                currentTotemStep = TotemStep.CLOSE;
                            } else {
                                resetAll();
                            }
                        } else {
                            ...
                            if (targetRefillSlot != -1) {
                                aimSlot(targetRefillSlot);
                                currentTotemStep = TotemStep.DO_REFILL;
                            } else {
                                if (autoEsc.getValue()) {
                                    currentTotemStep = TotemStep.CLOSE;
                                } else {
            ...
            case DO_REFILL:
                if (mc.currentScreen instanceof InventoryScreen) {
                    if (now - lastTotemTime >= delay) {
                        if (targetRefillSlot != -1) {
                            mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, targetRefillSlot, totemIdx, SlotActionType.SWAP, mc.player);
                        }
                        if (autoEsc.getValue()) {
                            currentTotemStep = TotemStep.CLOSE;
                        } else {
